...
export class LAppWavFileHandler {
  ...
  //the modifications on the WAV file are done right here
  public loadWavFile(filePath: string): boolean {
    ...
    const asyncFileLoad = async () => {
      return fetch(filePath).then(response => {
        //the main problem here was that we were unable to read the WAV file
        return response.arrayBuffer();
      });
    };

    const asyncWavFileManager = (async () => {
      //reading the file as an arraybuffer
      //instead of using the async load function, we s
